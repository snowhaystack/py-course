...
class Vehicle(Transport):
    ...
    def __init__(self,tire_num):
        ...
        self.add_garage()
        self._SERIAL = "-".join([str(random.randint(1,1000)) for x in range(10)])
    def start(self):
        print(id(self))
    def stop(self):
        print('Stop')
    ...
    def SERIAL(self):
        return self._SERIAL
    @SERIAL.setter #getter
    def SERIAL(self,num):
        self._SERIAL = "-".join([str(random.randint(1,1000)) for x in range(num)])
    def __del__(self):
...
print(Truck.garage)
print(f'truck SERIAL is {iveco.SERIAL}')

iveco.SERIAL = 50 #use setter eand getter
print(f'truck SERIAL is {iveco.SERIAL}')